  ...
  unsubscribeChat = db.collection("messages").doc(roomName).collection("chat").orderBy("time")
    .onSnapshot(snap => {
      ...
      snap.forEach(d => {
        ...
        if (m.text) {
          body = m.text;
        } else if (m.file && m.magnetURI) {
          body = `📁 ${m.file} <button onclick="downloadFile('${m.magnetURI}', '${m.file}')">Download</button>`;
        } else {
          body = "[Encrypted]";
        }
...
function downloadFile(magnetURI, filename) {
  client.add(magnetURI, torrent => {
    torrent.files[0].getBlob((err, blob) => {
      if (err) return alert("Error downloading file");
...
function loadFriends() {
  document.getElementById("friendsList").innerHTML = `<i>Coming soon: Threaded Chats</i>`;
}
